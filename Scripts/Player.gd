...
func _ready():
  ...
  if start_with_spawn_checkpoint:
...
@export var start_with_spawn_checkpoint = false
...
func die():
  ...
  if has_checkpoint:
    ...
    # Change to death screen scene when no checkpoint has been reached
    get_tree().change_scene_to_file("res://Scenes/DeathScreen.tscn")

func set_checkpoint(position: Vector2):
  checkpoint_position = position
  has_checkpoint = true
  print("Checkpoint updated: ", checkpoint_position)

func respawn():
  print("Respawning player at checkpoint: ", checkpoint_position)
  is_invincible = false
  if invincibility_timer:
